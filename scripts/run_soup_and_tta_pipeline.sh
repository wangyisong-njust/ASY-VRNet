...
GREEDY_OUT=weights/final_greedy_soup.pth
...
[[ -f weights/multiscale_ft_best.pth ]] || { log "ERROR: weights/multiscale_ft_best.pth missing"; exit 1; }

# ---- 1) greedy soup (subset selection) ----
# Candidates: the multi-scale fine-tune + the quality-aligned model's best and
# its late-epoch checkpoints (the greedy-soup ingredients), all under weights/.
...
CUDA_VISIBLE_DEVICES=${GPU} "${PY}" eval_paper_metrics.py \
    ...
    --out_dir results/greedy_soup_full 2>&1 | tail -4
...
CUDA_VISIBLE_DEVICES=${GPU} "${PY}" eval_paper_metrics.py \
    ...
    --out_dir results/greedy_soup_tta_320_384 2>&1 | tail -4
...
CUDA_VISIBLE_DEVICES=${GPU} "${PY}" eval_paper_metrics.py \
    ...
    --out_dir results/greedy_soup_tta_320_384_448 2>&1 | tail -4

# ---- summary ----
log "=== SUMMARY (baseline 42.570 | innov2 49.958 | old soup 50.114 | old TTA 51.186) ==="
for pair in \
    "results/greedy_soup_full:创新点二 greedy-soup" \
    "results/greedy_soup_tta_320_384:创新点三 TTA-2scale" \
    "results/greedy_soup_tta_320_384_448:TTA-3scale"; do
    d="${pair%%:*}"; name="${pair##*:}"
