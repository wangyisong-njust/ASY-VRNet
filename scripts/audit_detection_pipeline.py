...
def audit_radar(
            ...
            stats.append(
                {
                    ...
                    "loaded_channel_means": [float(np.mean(loaded[c][loaded[c] != 0])) if np.any(loaded[c] != 0) else 0.0 for c in range(loaded.shape[0])],
                }
            )
    ...
    return {
        ...
        "sample_stats": stats,
        "source_order": os.environ.get("ASY_RADAR_SOURCE_ORDER", "range,doppler,elevation,power"),
        "target_order": os.environ.get("ASY_RADAR_TARGET_ORDER", "range,elevation,velocity,power"),
        "interpretation": (
            ...
            "Legacy NPZ files are interpreted as range,doppler,elevation,power and remapped to paper REVP "
            "order range,elevation,velocity,power during loading."
        ),
    }
...
def write_reports(result: dict, out_dir: Path) -> tuple[Path, Path]:
    ...
    lines = [
        ...
        f"- Radar source order: {result['radar']['source_order']}",
        f"- Radar target order: {result['radar']['target_order']}",
        "",
        "## Summary",
        "",
