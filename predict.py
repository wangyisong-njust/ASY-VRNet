...
def parse_args():
    parser = argparse.ArgumentParser(description="Run ASY-VRNet detection on image/radar pairs.")
    parser.add_argument("--mode", default="predict",
                        choices=["predict", "dir_predict", "fps", "heatmap", "export_onnx", "video"])
    parser.add_argument("--image", default="", help="Image path for predict/fps/heatmap modes. Empty uses interactive input.")
    parser.add_argument("--output", default="", help="Optional output image path for predict mode.")
    parser.add_argument("--dir_origin_path", default="img/")
    parser.add_argument("--dir_save_path", default="img_out/")
    ...
    parser.add_argument("--radar_root", default=os.environ.get("ASY_RADAR_ROOT", ""))
    ...
    parser.add_argument("--onnx_save_path", default="model_data/models.onnx")
    parser.add_argument("--simplify", action="store_true", default=True)
    parser.add_argument("--no_simplify", action="store_false", dest="simplify")
    return parser.parse_args()


def yolo_kwargs(args):
    kwargs = {}
    for key in ["model_path", "classes_path", "radar_root", "input_shape", "phi", "confidence", "nms_iou"]:
...
if __name__ == "__main__":
    ...
    fps_image_path = args.image or "images/example1.jpg"
    # -------------------------------------------------------------------------#
    ...
    if mode == "predict":
        ...
        '''
        while True:
            img = args.image or input('Input image filename:')
            try:
                image = Image.open(img)
                image_id = image_id_from_path(img)
            except:
        ...
        image_id = image_id_from_path(fps_image_path)
